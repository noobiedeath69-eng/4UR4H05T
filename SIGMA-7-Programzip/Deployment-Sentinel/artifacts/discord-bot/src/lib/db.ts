...
export async function getActiveDeploymentRecord(guildId: string) {
  const [record] = await db.select().from(deploymentsTable).where(and(eq(deploymentsTable.guildId, guildId), eq(deploymentsTable.active, true))).limit(1);
  return record ?? null;
}

export async function updateDeploymentPhase(id: number, phase: string, extra: any = {}) {
  await db.update(deploymentsTable).set({ phase, ...extra }).where(eq(deploymentsTable.id, id));
}

export async function endDeploymentRecord(id: number) {
  await db.update(deploymentsTable).set({ active: false, phase: "ended", endedAt: new Date() }).where(eq(deploymentsTable.id, id));
}

export async function isUserWhitelisted(userId: string): Promise<boolean> {
  const [record] = await db.select().from(whitelistedUsersTable).where(eq(whitelistedUsersTable.userId, userId)).limit(1);
  return !!record;
}

export async function getSentientChannel(guildId: string) {
  const [record] = await db.select().from(sentientChannelsTable).where(eq(sentientChannelsTable.guildId, guildId)).limit(1);
  return record ?? null;
}

export async function setSentientChannel(guildId: string, channelId: string, channelName: string) {
  await db.insert(sentientChannelsTable).values({ guildId, channelId, channelName }).onConflictDoUpdate({
    target: sentientChannelsTable.channelId, set: { channelId, channelName, guildId }
  });
}
